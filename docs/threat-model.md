...
|---|---|---|---|
| tenant | IDOR / forged IDs coax a service method cross-tenant | App-layer `WHERE tenant_id` **plus** Postgres RLS `ENABLE`+`FORCE` on every tenant table *except the `device_telemetry` hypertable and its aggregates* (next row), GUC set per-tx by `db.WithTenant`; three DB roles (app `NOBYPASSRLS`, admin `BYPASSRLS`, mqtt narrow-grant) | The `pools-app-guard` CI check is textual on `pools.App.`; a pool passed under another field name evades it (the way alert delivery silently died once). Review-caught only. |
