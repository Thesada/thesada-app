...
| ten wrong codes on the claim form lock that device id until the earliest verified row announces the token already stored for it | the code is short on purpose. The counter is the worst `claim_failures` across the id's rows, so a squatter row cannot hide a locked real one, and the lookup locks those rows. An unverified announce, or a later verified key, does not clear the others. A different token does not |
...
spend that client's own cap, and the two caps are reserved together so
a lost client slot does not spend the address. One client cannot spend
the address cap alone. The handler still returns the same sent result,
